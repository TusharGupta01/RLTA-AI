...
class DAgger:
	...
	def agent_wapper(self, config, agent):
	    def fn(obs):
	        with torch.no_grad():
	...
	def train(self, config, agent, expert):
		expert_obs, expert_actions, *_ = self.run_agent(config, expert, config.n_expert_rollouts)
		...
		for k in range(config.n_dagger_iter):
			# training agent
			self.fit_dataset(config, agent, dataset, config.epochs)
			
			# run agent to get new on-policy observations
			new_obs, *_ = run_agent(config, self.agent_wapper(config, agent), config.n_dagger_rollouts)
		...
		return self.agent_wapper(config, agent)
	...
	def save_model(self, config, model, PATH):
	...
	def load_model(self, config, model, PATH):
	    PATH = PATH + config.envname + '-' + 'parameters.tar'
